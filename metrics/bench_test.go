...
func BenchmarkRecordMetric_Cached(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		config.RecordMetric(ctx, "test.cached.metric", float64(i))
	}
}
...
func BenchmarkRecordMetric_CachedWithAttributes(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		config.RecordMetric(ctx, "test.cached.with.attrs", float64(i), attrs...)
	}
}
...
func BenchmarkStartFinishRequest(b *testing.B) {
	...
	for b.Loop() {
		...
		config.FinishRequest(ctx, metrics, 200, 2048)
	}
}
...
func BenchmarkExcludedPath(b *testing.B) {
	...
	b.Run("NonExcludedPath", func(b *testing.B) {
		...
		for b.Loop() {
			...
			config.FinishRequest(ctx, metrics, 200, 1024)
		}
	})
}
