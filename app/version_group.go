...
// related routes under a specific API version. Version groups support app.HandlerFunc
// (with app.Context), providing access to binding, validation, and logging features.
//
...
//
// Any middleware added via Use() will be executed before the handler.
//
// Example:
//
...
//
// Middleware added via Use() will be applied to all subsequent routes registered
...
// The new group's prefix will be combined with the provided prefix.
// Middleware from the parent group is inherited by the nested group.
//
// Note: The returned router.VersionGroup does not support app.HandlerFunc.
