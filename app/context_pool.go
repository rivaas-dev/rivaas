...
// contextPool provides pooling for app.Context instances to minimize allocations.
...
//
// Note: Cleanup is also performed in wrapHandler's defer to ensure
...
func buildRequestLogger(base *slog.Logger, rc *router.Context) *slog.Logger {
	// Use no-op logger if none configured (singleton, no allocation)
	if base == nil {
		base = noopLogger
	}
