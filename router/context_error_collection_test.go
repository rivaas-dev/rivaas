...
// TestContext_ErrorCollection_ConcurrentSafety tests concurrent error collection
// Note: This test intentionally uses concurrent access, so we don't use t.Parallel()
// to avoid interference with other tests
func TestContext_ErrorCollection_ConcurrentSafety(t *testing.T) {
	w := httptest.NewRecorder()
	req := httptest.NewRequest("GET", "/", nil)
	c := NewContext(w, req)
	...
	// Should have collected errors (may not be exactly 10 due to race conditions,
	// but should have some errors)
	assert.True(t, c.HasErrors(), "Expected errors after concurrent collection")

	// Verify we have some errors
	collectedErrors := c.Errors()
	assert.NotEmpty(t, collectedErrors, "Expected at least some errors after concurrent collection")
}
