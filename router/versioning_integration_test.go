...
var _ = Describe("Versioning Integration", func() {
	Describe("Header-based versioning", func() {
		...
		BeforeEach(func() {
			r = router.MustNew(router.WithVersioning(
				router.WithHeaderVersioning("X-API-Version"),
				router.WithDefaultVersion("v1"),
				router.WithValidVersions("v1", "v2"),
			))
		})
		...
		Describe("Versioned routing", func() {
			...
			DescribeTable("HTTP methods with version header",
				...
				Entry("v1 GET", "GET", "/users", "v1", "v1 users", http.StatusOK),
				Entry("v2 GET", "GET", "/users", "v2", "v2 users", http.StatusOK),
				Entry("v1 POST", "POST", "/users", "v1", "v1 user created", http.StatusCreated),
				Entry("v2 POST", "POST", "/users", "v2", "v2 user created", http.StatusCreated),
				Entry("v1 PUT", "PUT", "/users/123", "v1", "v1 updated user 123", http.StatusOK),
				Entry("v1 DELETE", "DELETE", "/users/456", "v1", "v1 deleted user 456", http.StatusOK),
				Entry("v1 PATCH", "PATCH", "/users/789", "v1", "v1 patched user 789", http.StatusOK),
				Entry("v1 OPTIONS", "OPTIONS", "/users", "v1", "v1 options", http.StatusOK),
				Entry("v1 HEAD", "HEAD", "/users", "v1", "", http.StatusOK),
			)
		})
		...
		Describe("Versioned groups", func() {
			...
			DescribeTable("HTTP methods with versioned groups",
				...
				Entry("GET", "GET", "/api/users", "v1 api users", http.StatusOK),
				Entry("POST", "POST", "/api/users", "v1 api user created", http.StatusCreated),
				Entry("PUT", "PUT", "/api/users/123", "v1 api updated 123", http.StatusOK),
				Entry("DELETE", "DELETE", "/api/users/456", "v1 api deleted 456", http.StatusOK),
				Entry("PATCH", "PATCH", "/api/users/789", "v1 api patched 789", http.StatusOK),
				Entry("OPTIONS", "OPTIONS", "/api/users", "v1 api options", http.StatusOK),
				Entry("HEAD", "HEAD", "/api/users", "", http.StatusOK),
			)
		})
	...
	Describe("Path-based versioning", func() {
		...
		Describe("Path with API prefix", func() {
			...
			BeforeEach(func() {
				r = router.MustNew(router.WithVersioning(
					router.WithPathVersioning("/api/v{version}/"),
					router.WithDefaultVersion("v1"),
				))
		...
		Describe("Path stripping edge cases", func() {
			It("handles no path-based versioning", func() {
				...
				r := router.MustNew(router.WithVersioning(
					router.WithHeaderVersioning("X-API-Version"), // Only header versioning
					router.WithDefaultVersion("v1"),
				))
			...
			It("handles prefix extending beyond path", func() {
				...
				r := router.MustNew(router.WithVersioning(
					router.WithPathVersioning("/very/long/prefix/v{version}/"),
					router.WithDefaultVersion("v1"),
				))
	...
	Describe("Accept header versioning", func() {
		Describe("Basic accept versioning", func() {
			...
			BeforeEach(func() {
				r = router.MustNew(router.WithVersioning(
					router.WithAcceptVersioning("application/vnd.myapi.{version}+json"),
					router.WithDefaultVersion("v1"),
				))
		...
		Describe("Deprecated version headers", func() {
			...
			BeforeEach(func() {
				...
				r = router.MustNew(router.WithVersioning(
					router.WithHeaderVersioning("X-API-Version"),
					router.WithDefaultVersion("v1"),
					router.WithDeprecatedVersion("v1", sunsetTime),
				))
		...
		Describe("Multiple deprecated versions", func() {
			...
			BeforeEach(func() {
				...
				r = router.MustNew(router.WithVersioning(
					...
					router.WithDeprecatedVersion("v1", sunset1),
					router.WithDeprecatedVersion("v2", sunset2),
				))
		...
		Describe("Multiple detection methods", func() {
			...
			BeforeEach(func() {
				...
				r = router.MustNew(router.WithVersioning(
					router.WithPathVersioning("/v{version}/"),
					router.WithHeaderVersioning("X-API-Version"),
					router.WithAcceptVersioning("application/vnd.api.{version}+json"),
					router.WithQueryVersioning("v"),
					router.WithDefaultVersion("v1"),
					router.WithVersionObserver(
	...
	Describe("Complex Versioning Scenarios", func() {
		...
		BeforeEach(func() {
			...
			r = router.MustNew(router.WithVersioning(
				...
				router.WithValidVersions("v1", "v2", "v3"),
				router.WithDefaultVersion("v1"),
				router.WithDeprecatedVersion("v1", sunsetV1),
				router.WithVersionObserver(
			...
			for _, ver := range []string{"v1", "v2", "v3"} {
				...
				vr.GET("/users", func(c *router.Context) {
					c.String(http.StatusOK, `version: %s`, c.Version())
				})
			}
		})
		...
		Describe("Query parameter versioning", func() {
			...
			It("handles invalid query version", func() {
				req := httptest.NewRequest("GET", "/users?v=invalid", nil)
				w := httptest.NewRecorder()
				r.ServeHTTP(w, req)
