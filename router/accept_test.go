...
func TestAccepts_CacheHit(t *testing.T) {
	t.Run("cached_specs_reused", func(t *testing.T) {
		...
		if result1 != "json" {
			t.Errorf("First Accepts() call = %q, want %q", result1, "json")
		}

		// Verify cache was populated
		if c.cachedAcceptHeader != acceptHeader {
			t.Errorf("cachedAcceptHeader = %q, want %q", c.cachedAcceptHeader, acceptHeader)
		}
		if c.cachedAcceptSpecs == nil {
			t.Error("cachedAcceptSpecs should be populated after first call")
		}
		if len(c.cachedAcceptSpecs) == 0 {
			t.Error("cachedAcceptSpecs should contain parsed specs")
		}

		// Second call with same Accept header - should use cached specs
		result2 := c.Accepts("json", "html", "xml")
		if result2 != "json" {
			t.Errorf("Second Accepts() call = %q, want %q", result2, "json")
		}

		// Verify cache is still valid
		if c.cachedAcceptHeader != acceptHeader {
			t.Errorf("cachedAcceptHeader changed to %q, should still be %q", c.cachedAcceptHeader, acceptHeader)
		}
		if c.cachedAcceptSpecs == nil {
			t.Error("cachedAcceptSpecs should still be valid after second call")
		}

		// Third call with different offers but same header - should still use cache
		result3 := c.Accepts("html", "json")
		if result3 != "json" {
			t.Errorf("Third Accepts() call = %q, want %q", result3, "json")
		}
	})
	...
	t.Run("cache_invalidated_on_different_header", func(t *testing.T) {
		...
		if result1 != "json" {
			t.Errorf("First Accepts() = %q, want %q", result1, "json")
		}
		...
		if originalCachedSpecsLen == 0 {
			t.Fatal("cachedAcceptSpecs should be populated")
		}
		...
		if result2 != "html" {
			t.Errorf("Second Accepts() with different header = %q, want %q", result2, "html")
		}

		// Verify cache was updated
		if c.cachedAcceptHeader != "text/html" {
			t.Errorf("cachedAcceptHeader = %q, want %q", c.cachedAcceptHeader, "text/html")
		}
		if c.cachedAcceptHeader == originalCachedHeader {
			t.Error("cachedAcceptHeader should be different after header change")
		}
		if c.cachedAcceptSpecs == nil {
	...
	t.Run("multiple_cache_hits_same_request", func(t *testing.T) {
		...
		if c.cachedAcceptSpecs == nil {
			t.Error("cachedAcceptSpecs should be populated")
		}
	})
	...
	t.Run("cache_nil_specs_handled", func(t *testing.T) {
		...
		result1 := c.Accepts("json", "html")
		if result1 != "json" {
			t.Errorf("Accepts() with empty header = %q, want %q", result1, "json")
		}

		// Verify empty header doesn't cache specs (early return, not parsing)
		if c.cachedAcceptHeader != "" {
			t.Errorf("cachedAcceptHeader = %q, want empty", c.cachedAcceptHeader)
		}
	})
}
...
func TestHeaderArena_GetSpecs(t *testing.T) {
	t.Run("within_buffer_size_uses_arena", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if result == nil {
					t.Error("getSpecs should not return nil")
				}

				if cap(result) < tt.capacity {
					t.Errorf("getSpecs(%d) returned slice with capacity %d, want at least %d",
						tt.capacity, cap(result), tt.capacity)
				}

				if len(result) != 0 {
					t.Errorf("getSpecs(%d) returned slice with length %d, want 0",
						tt.capacity, len(result))
				}

				// Verify it uses the arena buffer (should be <= 16 capacity for arena buffer)
				// When using arena, capacity should be exactly the buffer size or less
				if cap(result) > 16 {
					t.Errorf("getSpecs(%d) returned slice with capacity %d > 16, should use arena buffer",
						tt.capacity, cap(result))
				}
			})
		}
	})
	...
	t.Run("exceeds_buffer_size_heap_alloc", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if result == nil {
					t.Error("getSpecs should not return nil even for large capacity")
				}
				...
				// Verify it's a heap-allocated slice (capacity > 16)
				if cap(result) <= 16 {
					t.Errorf("getSpecs(%d) returned slice with capacity %d <= 16, should use heap allocation",
						tt.capacity, cap(result))
				}

				// Verify the arena buffer is not affected
				if arena.used != 0 {
					t.Errorf("arena.used should be 0 after getSpecs, got %d", arena.used)
				}
			})
		}
	})
	...
	t.Run("multiple_calls_reset_used", func(t *testing.T) {
		...
		if arena.used != 0 {
			t.Errorf("getSpecs should reset arena.used to 0, got %d", arena.used)
		}
	...
	t.Run("end_to_end_with_large_header", func(t *testing.T) {
		...
		specs := parseAcceptFast(largeHeader, arena)

		if len(specs) != 20 {
			t.Errorf("parseAcceptFast with 20 parts returned %d specs, want 20", len(specs))
		}

		// Verify all specs are parsed correctly
		for i, spec := range specs {
			if spec.value != "application/json" {
				t.Errorf("spec[%d].value = %q, want %q", i, spec.value, "application/json")
			}
		}
	})
}
...
// TestParseAcceptPartFast tests parseAcceptPartFast with various inputs.
func TestParseAcceptPartFast(t *testing.T) {
	t.Run("empty_or_whitespace_single_part", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				result := parseAcceptPartFast(tt.input)

				if result.value != tt.expected.value {
					t.Errorf("parseAcceptPartFast(%q).value = %q, want %q",
						tt.input, result.value, tt.expected.value)
				}

				if result.quality != tt.expected.quality {
					t.Errorf("parseAcceptPartFast(%q).quality = %f, want %f",
						tt.input, result.quality, tt.expected.quality)
				}

				if result.params != nil && tt.expected.params == nil {
					t.Errorf("parseAcceptPartFast(%q).params = %v, want nil",
						tt.input, result.params)
				}

				if result.rawQuality != tt.expected.rawQuality {
					t.Errorf("parseAcceptPartFast(%q).rawQuality = %q, want %q",
						tt.input, result.rawQuality, tt.expected.rawQuality)
				}
			})
		}
	})
	...
	t.Run("empty_parts_in_full_header", func(t *testing.T) {
		...
		// by parseAcceptFast (which checks spec.value != "").
		tests := []struct {
			name           string
			header         string
			expectedCount  int
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if len(specs) != tt.expectedCount {
					t.Errorf("parseAcceptFast(%q) returned %d specs, want %d",
						tt.header, len(specs), tt.expectedCount)
				}

				if len(specs) != len(tt.expectedValues) {
					t.Fatalf("spec count mismatch: got %d, want %d", len(specs), len(tt.expectedValues))
				}

				for i, spec := range specs {
					if spec.value != tt.expectedValues[i] {
						t.Errorf("spec[%d].value = %q, want %q",
							i, spec.value, tt.expectedValues[i])
					}
				}
			})
		}
...
// TestParseAcceptFast_EmptyHeader tests parseAcceptFast with empty header.
...
func TestParseAcceptFast_EmptyHeader(t *testing.T) {
	t.Run("empty_header_returns_nil", func(t *testing.T) {
		...
		result := parseAcceptFast("", arena)

		if result != nil {
			t.Errorf("parseAcceptFast(\"\") = %v, want nil", result)
		}

		if len(result) != 0 {
			t.Errorf("parseAcceptFast(\"\") returned slice with length %d, want 0 or nil", len(result))
		}
	})
	...
	t.Run("empty_header_early_return_before_arena_usage", func(t *testing.T) {
		...
		result := parseAcceptFast("", nil)

		if result != nil {
			t.Errorf("parseAcceptFast(\"\") with nil arena = %v, want nil", result)
		}
	})
	...
	t.Run("empty_header_vs_whitespace_header", func(t *testing.T) {
		...
		resultEmpty := parseAcceptFast("", arena)
		if resultEmpty != nil {
			t.Errorf("parseAcceptFast(\"\") = %v, want nil", resultEmpty)
		}

		// Whitespace should return empty slice (not nil)
		arena.reset()
		resultWhitespace := parseAcceptFast("   ", arena)
		if resultWhitespace == nil {
			t.Error("parseAcceptFast(\"   \") returned nil, want empty slice")
		}
		if len(resultWhitespace) != 0 {
			t.Errorf("parseAcceptFast(\"   \") returned slice with length %d, want 0", len(resultWhitespace))
		}
	})
}

// TestParseParamFast tests parseParamFast function, specifically covering early return cases.
func TestParseParamFast(t *testing.T) {
	t.Run("empty_or_whitespace_only_returns_early", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				parseParamFast(tt.param, &spec)

				// Spec should be unchanged (early return for empty/whitespace)
				if spec.value != originalSpec.value {
					t.Errorf("spec.value changed from %q to %q, should be unchanged",
						originalSpec.value, spec.value)
				}
				if spec.quality != originalSpec.quality {
					t.Errorf("spec.quality changed from %f to %f, should be unchanged",
						originalSpec.quality, spec.quality)
				}
				if spec.params != nil {
					t.Errorf("spec.params should be nil, got %v", spec.params)
				}
			})
		}
	})
	...
	t.Run("parseFloat_fallback_lines_376_379", func(t *testing.T) {
		// Test the fallback path when parseQFast fails but ParseFloat succeeds
		...
		tests := []struct {
			...
			{
				name:           "q value exactly 1.0 - should use fast path",
				param:          "q=1.0",
				expectedResult: "set",
				expectedValue:  1.0,
			},
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if tt.expectedResult == "set" {
					...
					if spec.quality != tt.expectedValue {
						t.Errorf("expected quality %f, got %f",
							tt.expectedValue, spec.quality)
					}
					if spec.rawQuality == "" {
						t.Error("rawQuality should be set")
					}
				} else {
					// Quality should remain unchanged (fallback parsed but value out of range)
					if spec.quality != tt.expectedValue {
						t.Errorf("expected quality to remain %f (unchanged), got %f",
							tt.expectedValue, spec.quality)
					}
				}
			})
	...
	t.Run("valid_parameters_parsed_correctly", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if tt.expectQParam {
					if spec.quality != tt.expectedQValue {
						t.Errorf("q parameter: expected quality %f, got %f",
							tt.expectedQValue, spec.quality)
					}
					if spec.rawQuality == "" {
						t.Error("q parameter: rawQuality should be set")
					}
				} else {
					if spec.params == nil {
						t.Fatal("params map should be initialized")
					}
					if val, ok := spec.params[tt.expectOtherKey]; !ok {
						t.Errorf("expected key %q not found in params", tt.expectOtherKey)
					} else if val != tt.expectedOtherValue {
						t.Errorf("expected value %q for key %q, got %q",
							tt.expectedOtherValue, tt.expectOtherKey, val)
					}
				}
			})
		}
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if typeStr != tt.expectedType {
					t.Errorf("splitMediaType(%q).type = %q, want %q",
						tt.mediaType, typeStr, tt.expectedType)
				}

				if subtypeStr != tt.expectedSubtype {
					t.Errorf("splitMediaType(%q).subtype = %q, want %q",
						tt.mediaType, subtypeStr, tt.expectedSubtype)
				}
			})
		}
	})
	...
	t.Run("no_slash_fallback", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if subtypeStr != tt.expectedSubtype {
					t.Errorf("splitMediaType(%q).subtype = %q, want %q (should be \"*\" when no slash)",
						tt.mediaType, subtypeStr, tt.expectedSubtype)
				}
			})
		}
	...
	t.Run("unknown_short_name_fallback", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if result != tt.expected {
					t.Errorf("normalizeMediaType(%q) = %q, want %q\nDescription: %s",
						tt.mediaType, result, tt.expected, tt.description)
				}
			})
		}
	})
	...
	t.Run("known_short_names", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if result != tt.expected {
					t.Errorf("normalizeMediaType(%q) = %q, want %q",
						tt.mediaType, result, tt.expected)
				}
			})
		}
	})
	...
	t.Run("edge_cases_covering_all_paths", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if result != tt.expected {
					t.Errorf("normalizeMediaType(%q) = %q, want %q\nPath: %s",
						tt.mediaType, result, tt.expected, tt.path)
				}
			})
		}
	})
	...
	t.Run("empty_offers_returns_empty", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				if result != tt.expected {
					t.Errorf("acceptHeaderMatch(specs, offers) = %q, want %q\nSpecs: %v\nOffers: %v",
						result, tt.expected, tt.specs, tt.offers)
				}
			})
		}
	})
