...
type config struct {
	...
	// We use map[string]bool instead of []string for O(1) lookup performance,
	// since this check happens on every request in the hot path.
...
//
// Security Features:
//   - Dual validation: Checks Content-Length header (fast fail) AND actual bytes read (secure)
...
// read, which makes it super secure. The Content-Length check provides fast rejection for
// obviously oversized requests, while the limitedReader wrapper ensures we never read more
// than the limit, even if the header is incorrect or missing.
//
// Basic usage:
//
//	r := router.New()
...
// How it works:
//
//  1. First phase: Check Content-Length header (fast rejection)
...
//   - Content-Length header check is fast but can be spoofed - actual read is secure
...
//
// Performance: This middleware adds minimal overhead (~50-100ns per request).
// The Content-Length check is O(1), and the limitedReader adds ~1-2ns per Read() call.
//
...
func New(opts ...Option) router.HandlerFunc {
	...
	return func(c *router.Context) {
		...
		// Phase 1: Fast check - validate Content-Length header
		// This provides quick rejection for obviously oversized requests
