...
func setFieldValue(field reflect.Value, value string, opts *Options) error {
	...
	// Priority 2: Check for encoding.TextUnmarshaler interface (fast path)
...
func setSliceField(field reflect.Value, values []string, opts *Options) error {
	...
	if opts.maxSliceLen > 0 && len(values) > opts.maxSliceLen {
		return fmt.Errorf("slice exceeds max length: %d > %d (use WithMaxSliceLen to increase)",
			len(values), opts.maxSliceLen)
	}
...
// parseBoolGenerous parses various boolean string representations (generous parsing).
...
// Tries default formats first, then custom layouts from options.
...
func validateEnum(value string, enumValues string) error {
	...
	for _, a := range strings.Split(enumValues, ",") {
...
func setMapField(field reflect.Value, getter ValueGetter, prefix string, fieldType reflect.Type, opts *Options) error {
	...
	if opts.maxMapSize > 0 && capacity > opts.maxMapSize {
		return fmt.Errorf("map exceeds max size: %d > %d (use WithMaxMapSize to increase)",
			capacity, opts.maxMapSize)
	}
...
// findConverter locates a registered converter with pointer/interface normalization.
...
// estimateMapCapacity uses approxSizer interface to avoid type assertions.
