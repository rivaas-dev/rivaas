...
// TestIntegration_YAMLWithValidator tests YAML binding with validation
func TestIntegration_YAMLWithValidator(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test in short mode")
	}
	t.Parallel()
	...
	// Create a simple validator that validates port range
	validator := &portValidator{minPort: 1024, maxPort: 65535}
...
// portValidator is a simple validator for testing
type portValidator struct {
	minPort int
	maxPort int
}

func (v *portValidator) Validate(data any) error {
