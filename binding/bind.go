...
// SourceConfig maps a tag name to its ValueGetter.
// Used by BindMulti to bind from multiple sources based on struct tags.
type SourceConfig struct {
	Tag    string
	Getter ValueGetter
}

// BindMulti binds from multiple sources based on struct tags.
// It introspects the struct and only binds from sources where tags are present.
//
...
// HasStructTag checks if any field in the struct (including embedded structs)
// has the given tag. This is useful for determining which binding sources
// should be used when binding from multiple sources.
