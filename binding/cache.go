...
// getStructInfo retrieves or parses struct information (RCU read path).
// It uses an RCU (Read-Copy-Update) pattern for lock-free reads in the hot path.
//
...
//
// Returns:
//   - *structInfo: Cached metadata containing field information and validation rules
func getStructInfo(typ reflect.Type, tag string) *structInfo {
	// Defensive checks: fail fast on invalid input
	if typ == nil {
	...
	// Fast path: lock-free read from current map
...
//
// Invalid types are silently skipped. Use MustWarmupCache to fail fast on errors.
//
// Example:
//
...
// Use during application startup to fail fast on mis-tagged structs.
//
// Example:
//
//	func init() {
